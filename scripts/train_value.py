...
def load_checkpoint(checkpoint_path: Path, state: TrainState) -> TrainState:
    ...
    # 恢复参数。EMA-only checkpoint 用 EMA 同时初始化训练参数和 EMA，
    # 这样可以把 checkpoint 主机内存峰值降低约一半，并继续训练。
    if "params" in restored:
        state.params.replace_by_pure_dict(restored["params"])
    elif "ema_params" in restored:
        state.params.replace_by_pure_dict(restored["ema_params"])
    else:
        raise KeyError(f"Checkpoint 缺少 params/ema_params: {checkpoint_path}")
...
def save_checkpoint(
    ...
    with ocp.PyTreeCheckpointer() as ckptr:
        if checkpoint_mode == "ema_only":
            if state.ema_params is None:
                raise ValueError("checkpoint_mode=ema_only 需要 EMA 参数")
            save_dict = {
                "ema_params": state.ema_params.to_pure_dict(),
                "step": step,
            }
...
def main():
    ...
    parser.add_argument(
        "--checkpoint_mode",
        choices=("full", "ema_only"),
        default="full",
        help="full 保存训练参数和 EMA；ema_only 仅保存推理使用的 EMA，以降低保存时主机内存峰值",
    )

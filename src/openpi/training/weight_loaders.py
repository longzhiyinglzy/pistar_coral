...
@dataclasses.dataclass(frozen=True)
class CheckpointOverlayWeightLoader(WeightLoader):
    """Reconstruct a full initialization from a base checkpoint plus a strict parameter overlay.

    This is used by PiStar-CORAL to warm-start from an exported CORAL expert
    without transferring the larger full training checkpoint.
    """
    ...
    def load(self, params: at.Params) -> at.Params:
        base = CheckpointWeightLoader(self.base_params_path).load(params)
        overlay = _model.restore_params(
            download.maybe_download(self.overlay_params_path),
        ...
        if not flat_overlay:
            raise ValueError("Checkpoint overlay is empty.")

        unknown = sorted(set(flat_overlay) - set(flat_base))
        if unknown:
            raise KeyError(f"Overlay contains unknown parameter paths: {unknown[:10]}")

        shape_mismatch = sorted(
            key
            for key in flat_overlay
            if getattr(flat_overlay[key], "shape", None) != getattr(flat_base[key], "shape", None)
        )
        if shape_mismatch:
            raise ValueError(f"Overlay parameter shape mismatch: {shape_mismatch[:10]}")

        for key, value in flat_overlay.items():
            reference = flat_base[key]
            flat_base[key] = value.astype(reference.dtype) if value.dtype != reference.dtype else value

        logger.info(
            console.info(
                f"Checkpoint overlay: replaced {len(flat_overlay)} of {len(flat_base)} parameter leaves "
                f"from {self.overlay_params_path}"
            )
        )
